sap.ui.define([
	...
	return Controller.extend("SS.SigniwisSports.controller.ItemDetails", {
		...
		onClick: function (oEvent) {
			...
			var outerItem = path.split(".")[path.split(".").length - 3];
			var item = path.split(".")[path.split(".").length - 2];
			var itemIndex = path.split(".")[path.split(".").length - 1];
			}
			var objectPath = "/productTypes/" + outerItem + "/" + item + "/" + itemIndex;
			var object = this.getView().byId("object");
			object.bindElement(objectPath);
		}
